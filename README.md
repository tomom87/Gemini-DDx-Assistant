...
3.  **シームレスな UX**
    *   **オート実行**: 安全性が確認されたテキストは、サイドパネルを開くと同時に自動解析がスタートします。
    *   **オートコピー**: 解析完了時、カルテ記載用の要約テキストが自動的にクリップボードにコピーされます（Offscreen API 利用）。
    *   **詳細アラート**: 個人情報の可能性がある箇所を具体的に（氏名、IDなど）指摘します。
...
*   **API通信**: Google Gemini API への通信のみを行います。PubMed 等の外部データベースへの直接通信は行わず、全てのロジックは Gemini API 経由のみとなります。
